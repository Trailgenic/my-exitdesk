"""Verify the published capital allocation and deal affordability phase without modifying remote content."""
...
OUT = ROOT / 'site-foundation/capital-allocation-release/production-audit.json'
...
urls = list(dict.fromkeys([r['url'] for r in inventory['resources']] + [
    ...
    'https://mikeye.com/ma-resources/capital-allocation-deal-affordability',
...
check('Inventory contains 19 resources', len(inventory['resources']) == 19)
...
resource_url = next(r['url'] for r in inventory['resources'] if r['id']=='capital-allocation-deal-affordability')
...
for rid in ['acquisition-mandate-target-screen','loi-economics-risk-allocator','synergy-value-bridge','carve-out-perimeter-tsa-planner','deal-workflow']:
...
check('Methodology workbook link',inventory['resources'][next(i for i,r in enumerate(inventory['resources']) if r['id']=='capital-allocation-deal-affordability')]['download']['url'] in actual_links(resource_html))
...
capital_record=next(r for r in inventory['resources'] if r['id']=='capital-allocation-deal-affordability')
...
for url,suffix,count in [('https://www.mikeye.com/m-and-a','#resources',19),('https://www.mikeye.com/tools-and-models','#tools',14)]:
...
check('Inventory contains 13 workbook downloads', len(books) == 13)
