"""Verify the published LOI phase without modifying remote content."""
import concurrent.futures, datetime, hashlib, html, json, re, subprocess, tempfile
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / 'site-foundation/loi-release/production-audit.json'
...
urls = list(dict.fromkeys([r['url'] for r in inventory['resources']] + [
    ...
    'https://mikeye.com/ma-resources/loi-economics-risk-allocator',
...
resource_url = next(r['url'] for r in inventory['resources'] if r['id']=='loi-economics-risk-allocator')
...
for label,markup in [('M&A Library',library_html),('Tools & Models',by_url['https://www.mikeye.com/tools-and-models']['body'].decode())]:
    check(label+' links LOI guide',resource_url in markup or '/ma-resources/loi-economics-risk-allocator' in markup)
check('LOI sitemap inclusion',resource_url in by_url['https://www.mikeye.com/sitemap.xml']['body'].decode())
for rid in ['deal-workflow','workflow-diligence','integration-continuity','synergy-value-bridge']:
    u=next(r['url'] for r in inventory['resources'] if r['id']==rid)
    check('Reciprocal guide link: '+rid,'/ma-resources/loi-economics-risk-allocator' in by_url[u]['body'].decode() and u.replace('https://www.mikeye.com','') in resource_html)
