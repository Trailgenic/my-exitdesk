...
const ROOT='site-foundation', OUT=ROOT+'/capital-allocation-release', WORKER='../mikeye-workers', SITE='https://www.mikeye.com';
const ID='capital-allocation-deal-affordability';
...
const filename='MikeYe-Capital-Allocation-Deal-Affordability-v1.0.xlsx';
const repositoryPath='public/resources/'+filename;
const bytes=fs.readFileSync(repositoryPath);
book.download={filename,repositoryPath,url:`https://raw.githubusercontent.com/Trailgenic/my-exitdesk/${commit}/${repositoryPath}`,version:'1.0',bytes:bytes.length,sha256:crypto.createHash('sha256').update(bytes).digest('hex'),sheets:['Dashboard','Controls','Deal cash flow','Financing','Capacity','Alternatives','Guide'],downloadVerified:true,verifiedAt:'2026-09-10'};
...
intro.html=`<p><a href="${book.download.url}">Download the Capital Allocation &amp; Deal Affordability workbook</a> · Excel · Version 1.0 · USD millions</p>`+intro.html.replace(/^<p><a href="[^"]+">Download the Capital Allocation[\s\S]*?<\/p>/,'');
write(`${ROOT}/editorial/${ID}.json`,book);

const addition={heading:'Set the capital limit before committing',html:`<p>Use the <a href="${book.proposedPath}">Capital Allocation &amp; Deal Affordability Tool</a> to compare buying with the alternatives, test financing and downside cash demands, and separate the affordable price from the buyer's private hard walk-away price.</p>`};
...
manifest.version='2.5.0';manifest.updated='2026-09-10';manifest.phase=ID;
const stages=read(ROOT+'/transaction-path.json');
write(ROOT+'/transaction-path.json',stages);manifest.transactionPath=stages;
for(const r of manifest.resourceRoadmap){if(r.topic==='strategic-finance')r.status=book.status;if(r.topic==='industry-comps')r.status='next';}
...
inventory.version='2.5.0';inventory.updated='2026-09-10';inventory.transactionPath=stages;
...
const resource={'@type':['CreativeWork','LearningResource'],'@id':book.url+'#resource',url:book.url,name:book.name,description:book.summary,author:{'@id':SITE+'/#person'},publisher:{'@id':SITE+'/#person'},inLanguage:'en-US',isAccessibleForFree:true,isPartOf:{'@id':SITE+'/m-and-a#library'},about:{'@id':SITE+'/m-and-a#'+book.topic},learningResourceType:'Capital allocation, acquisition affordability, and price ceiling tool with Excel workbook',teaches:book.decision,version:book.version,datePublished:book.publicationDate,dateModified:book.contentReviewedAt,mainEntityOfPage:{'@id':book.url+'#webpage'},encoding:{'@type':'MediaObject','@id':book.url+'#workbook',name:filename,contentUrl:book.download.url,encodingFormat:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',contentSize:book.download.bytes+' bytes'},keywords:['capital allocation','deal affordability','acquisition financing','downside survivability','opportunity cost','ROIC','IRR','payback','walk-away price'],citation:book.relatedResources.map(id=>({'@id':inventory.resources.find(r=>r.id===id).url+'#resource'}))};
...
for(const page of pages){
  ...
  for(const n of g){
    if(n['@type']==='CollectionPage'){n.dateModified='2026-09-10';n.description=page.publishedPath==='/m-and-a'?'Seven transaction stages and ten M&A knowledge pillars with practical tools for capital allocation, acquisition mandates, carve-outs, deal economics, diligence, integration, and valuation.':'Excel tools for capital allocation, acquisition affordability, carve-outs, mandates, deal workflow, diligence, and integration, plus five company valuation models.';}
