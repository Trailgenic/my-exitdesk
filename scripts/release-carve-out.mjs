...
const ROOT='site-foundation', OUT=ROOT+'/carve-out-release', WORKER='../mikeye-workers', SITE='https://www.mikeye.com';
const ID='carve-out-perimeter-tsa-planner';
...
const filename='MikeYe-Carve-Out-Perimeter-TSA-Planner-v1.0.xlsx';
const repositoryPath='public/resources/'+filename;
const bytes=fs.readFileSync(repositoryPath);
book.download={filename,repositoryPath,url:`https://raw.githubusercontent.com/Trailgenic/my-exitdesk/${commit}/${repositoryPath}`,version:'1.0',bytes:bytes.length,sha256:crypto.createHash('sha256').update(bytes).digest('hex'),sheets:['Dashboard','Workflows','Perimeter','Dependencies','TSA','Costs','Guide'],downloadVerified:true,verifiedAt:'2026-09-10'};
...
intro.html=`<p><a href="${book.download.url}">Download the Carve-Out Perimeter &amp; TSA Planner workbook</a> · Excel · Version 1.0 · USD thousands</p>`+intro.html.replace(/^<p><a href="[^"]+">Download the Carve-Out Perimeter[\s\S]*?<\/p>/,'');
write(`${ROOT}/editorial/${ID}.json`,book);

const addition={heading:'Define the separation before relying on it',html:`<p>Use the <a href="${book.proposedPath}">Carve-Out Perimeter &amp; TSA Planner</a> to define what transfers, map seller dependencies, separate Day 1 continuity from independent operations, and give temporary services a priced scope, owner, and exit plan.</p>`};
...
manifest.version='2.4.0';manifest.updated='2026-09-10';manifest.phase=ID;
const stages=read(ROOT+'/transaction-path.json');
write(ROOT+'/transaction-path.json',stages);manifest.transactionPath=stages;
for(const r of manifest.resourceRoadmap){if(r.topic==='divestitures')r.status=book.status;if(r.topic==='industry-comps')r.status='next';}
...
inventory.version='2.4.0';inventory.updated='2026-09-10';inventory.transactionPath=stages;
...
const resource={'@type':['CreativeWork','LearningResource'],'@id':book.url+'#resource',url:book.url,name:book.name,description:book.summary,author:{'@id':SITE+'/#person'},publisher:{'@id':SITE+'/#person'},inLanguage:'en-US',isAccessibleForFree:true,isPartOf:{'@id':SITE+'/m-and-a#library'},about:{'@id':SITE+'/m-and-a#'+book.topic},learningResourceType:'Carve-out perimeter, TSA, and operating readiness tool with Excel workbook',teaches:book.decision,version:book.version,datePublished:book.publicationDate,dateModified:book.contentReviewedAt,mainEntityOfPage:{'@id':book.url+'#webpage'},encoding:{'@type':'MediaObject','@id':book.url+'#workbook',name:filename,contentUrl:book.download.url,encodingFormat:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',contentSize:book.download.bytes+' bytes'},keywords:['carve-out perimeter','transition services agreement','TSA exit','Day 1 readiness','standalone costs','stranded costs','separation costs'],citation:book.relatedResources.map(id=>({'@id':inventory.resources.find(r=>r.id===id).url+'#resource'}))};
...
for(const page of pages){
  ...
  for(const n of g){
    if(n['@type']==='CollectionPage'){n.dateModified='2026-09-10';n.description=page.publishedPath==='/m-and-a'?'Seven transaction stages and ten M&A knowledge pillars with practical tools for acquisition mandates, carve-outs, LOI economics, synergies, diligence, integration, and valuation.':'Excel tools for carve-outs, acquisition mandates, deal workflow, LOI economics, synergies, diligence, and integration, plus five company valuation models.';}
...
guide=guide.replace('an Acquisition Mandate & Target Screen, and the existing valuation calculator.','an Acquisition Mandate & Target Screen, a Carve-Out Perimeter & TSA Planner, and the existing valuation calculator.');
