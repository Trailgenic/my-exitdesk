"""Verify the published carve-out perimeter and TSA phase without modifying remote content."""
...
OUT = ROOT / 'site-foundation/carve-out-release/production-audit.json'
...
urls = list(dict.fromkeys([r['url'] for r in inventory['resources']] + [
    ...
    'https://mikeye.com/ma-resources/carve-out-perimeter-tsa-planner',
...
check('Inventory contains 18 resources', len(inventory['resources']) == 18)
...
resource_url = next(r['url'] for r in inventory['resources'] if r['id']=='carve-out-perimeter-tsa-planner')
...
for label,markup in [('M&A Library',library_html),('Tools & Models',by_url['https://www.mikeye.com/tools-and-models']['body'].decode())]:
    check(label+' links carve-out guide',resource_url in actual_links(markup))
check('Carve-out sitemap inclusion',resource_url in by_url['https://www.mikeye.com/sitemap.xml']['body'].decode())
for rid in ['deal-workflow','workflow-diligence','integration-continuity','loi-economics-risk-allocator','synergy-value-bridge']:
...
check('Methodology workbook link',inventory['resources'][next(i for i,r in enumerate(inventory['resources']) if r['id']=='carve-out-perimeter-tsa-planner')]['download']['url'] in actual_links(resource_html))
...
carve_record=next(r for r in inventory['resources'] if r['id']=='carve-out-perimeter-tsa-planner')
...
check('MediaObject matches verified workbook',
      ...
      and encoding.get('contentUrl') == carve_record['download']['url']
      and encoding.get('name') == carve_record['download']['url'].rsplit('/', 1)[-1]
      and encoding.get('encodingFormat') == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
      and ('contentSize' not in encoding or encoding['contentSize'] == str(carve_record['download']['bytes'])+' bytes'))
...
for url,suffix,count in [('https://www.mikeye.com/m-and-a','#resources',18),('https://www.mikeye.com/tools-and-models','#tools',13)]:
...
check('Inventory contains 12 workbook downloads', len(books) == 12)
