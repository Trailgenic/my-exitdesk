...
export async function POST(request: Request) {
  ...
  function resolveFormValue(field: string, value: string | undefined): string | null {
    ...
    const maps: Record<string, Record<string, string>> = {
      ...
      facility: { own_real_estate: "Owns real estate", secure_lease: "Secure facility — long-term lease or stable month-to-month tenancy with established landlord history", uncertain_lease: "Uncertain facility — short-term lease or recent month-to-month tenancy that may not transfer cleanly to a buyer", no_facility: "No fixed facility — home-based, mobile, or remote operation" },
